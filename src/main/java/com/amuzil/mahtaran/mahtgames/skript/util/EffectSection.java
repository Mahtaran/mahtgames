package com.amuzil.mahtaran.mahtgames.skript.util;
...
public abstract class EffectSection extends Condition {
	...
	public EffectSection() {
		if (this instanceof LazyEffectSection) //This one doesn't need to load the section separated.
			return;
		Node node = SkriptLogger.getNode(); //Skript sets the node before parsing this 'effect'
		if (!(node instanceof SectionNode)) //Check in case it wasn't loaded as inline condition
			return;
		...
		String comment = ReflectionUtils.getField(Node.class, node, "comment");
		if (comment == null)
			comment = "";
		...
		ReflectionUtils.setField(SectionNode.class, section, "nodes", ReflectionUtils.getField(SectionNode.class, node, "nodes"));
		//Then it will clear the nodes from the current node, so Skript won't parse it (you need to parse then later).
		ReflectionUtils.setField(SectionNode.class, node, "nodes", new ArrayList<Node>());
	}
